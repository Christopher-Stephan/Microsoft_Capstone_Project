...
from ctr_model import ctr_recommender  # New: CTR recommender function from ctr_model.py
from BERT_model import BERT_recommender  # BERT recommender function
# (LLM recommender code is assumed to be embedded in the LLM tab below)
...
# =====================================
# TABS: CTR, BERT, and LLM Recommenders
# =====================================
tabs = st.tabs(["CTR Recommender", "BERT Recommender", "LLM Recommender"])

# TAB 1: CTR Recommender (Interactive)
with tabs[0]:
    st.markdown('<div class="tab-content">', unsafe_allow_html=True)
    st.markdown('<h2 class="section-header">CTR Recommender 📊</h2>', unsafe_allow_html=True)
    st.markdown(
        '<p class="description-text">This method leverages click-through rate (CTR) modeling to recommend news based on user behavior patterns. Please enter your user ID for CTR recommendations.</p>',
        unsafe_allow_html=True
    )
    user_id_input_ctr = st.text_input("Enter a user ID for CTR recommendations:", value="U12345", key="ctr_user")
    
    if st.button("Get CTR Recommendations", key="ctr_button"):
        try:
            recommendations, explanation, _ = ctr_recommender(
                user_id=user_id_input_ctr,
...
# TAB 2: BERT Recommender (Content-Based)
with tabs[1]:
    st.markdown('<div class="tab-content">', unsafe_allow_html=True)
    ...
    user_id_input_cb = st.text_input("Enter a user ID for BERT recommendations:", value="U12345", key="cb_user")
...
with tabs[2]:
    ...
    st.markdown("""
        <p class="description-text">
            ...
            Your answers will be used to build your profile and recommend a dynamic number of articles with clickable titles.
        </p>
    """, unsafe_allow_html=True)
